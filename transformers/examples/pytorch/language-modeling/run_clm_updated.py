...
class ModelArguments:
    ...
    e2e_train: Optional[str] = field(
        default='/home/exouser/Capstone/Diffusion-LM-main/datasets/e2e_data',
        metadata={"help": "simple wiki path"},
    )
...
def main():
    ...
    ###################### LOAD DATASETS & dictionary #########################
        ...
        elif model_args.experiment in ['e2e-tgt-times', 'e2e-tgt-gen-times']:
            times_vocab = {'START': 0, 'END': 1, 'UNK': 2, 'PAD': 3}
            times_lst = ['morning', 'afternoon', 'evening', 'night',
                         'weekday', 'weekend', 'today', 'tomorrow',
                         'yesterday', 'next week', 'last year',
                         'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
            for x in times_lst:
                times_vocab[x] = len(times_vocab)
    ...
    if model_args.model_name_or_path:
        ...
        ############# LOAD MODELS for controllable classifier ##############
        elif model_args.experiment in ['e2e-back', 'e2e-back_t2', 'e2e-tgt-pos', 'e2e-tgt-length', 'e2e-tgt-times', 'e2e-tgt-tree']:
            import torch
            config.vocab_size = len(tokenizer)
            print('\n Initializing the model from scratch \n' + '*' * 100)
            ...
            elif model_args.experiment == 'e2e-tgt-times':
                config.times_vocab_size = len(times_vocab) 
                model = Classifier_Times(config=config, diffusion=diffusion, )
            #elif model_args.experiment == 'e2e-tgt-length':
             #   model = Classifier_Length(config=config, diffusion=diffusion, )
    ...
    if model_args.experiment.startswith('synth'):
        ...
        #changed to 
        #from utils_updated import chart_from_tree, remove_leaves, pad_charts
        #------------------------------------
        from utils_updated import chart_from_tree, remove_leaves, pad_charts
    ...
    #                              or model_args.experiment in ['synth_emb', 'pos_emb', 'roc_emb', 'simple-wiki_emb', 'e2e-tgt_emb'])
    #                              and model_args.task not in ['data_teacher', 'finetune']) \
    #                     else AutoTokenizer.from_pretrained(args.tokenizer_name)
    #------------------------------------
    trainer_tokenizer = None if ((model_args.experiment in ['pos', 'synth', 'roc', 'simple-wiki', 'e2e-tgt',
                                                            'e2e-tgt-pos','e2e-tgt-tree', 'e2e-back', 'e2e-back_t2']
                                 or model_args.experiment in ['synth_emb', 'pos_emb', 'roc_emb', 'simple-wiki_emb', 'e2e-tgt_emb'])
                                 and model_args.task not in ['data_teacher', 'finetune']) \
                        else AutoTokenizer.from_pretrained(model_args.tokenizer_name, **tokenizer_kwargs)
